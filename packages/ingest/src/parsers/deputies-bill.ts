  ...
  return $("a[href$='.pdf'], a[href*='.pdf?'], a[href*='/docs?']")
    ...
    .map((node, index) => {
      ...
      return {
        ...
        label,
        url,
        documentKind: classifyDeputiesDocumentKind(`${label} ${rowText} ${url}`),
...
export function classifyDeputiesDocumentKind(text: string): DocumentKind {
